...
describe('helpers', function() {
    describe('openCitationsHelper', function() {
        ...
        describe('queryByString', function(){
            it('should return result for a given query', function(done) {
                this.timeout(100000);
                openCitationsHelper.queryByQueryString("Circulation Research", function(err,result){
                    should.not.exists(err);
                    result.should.be.ok();
                    result.should.be.Array().and.have.lengthOf(1);
                    result[0].should.be.ok();
                    result[0].should.have.property("journalArticle_title", "Genetically Encoded Voltage Indicators in Circulation Research");
                    done();
                });
            });
        });
