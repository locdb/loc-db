...
// make a request
function GET (path, cacheDirectory, cb) {
  cachedRequest.setCacheDirectory(cacheDirectory);
  // console.log(`### ${endpoint}${path}`);
  cachedRequest({ url: `${endpoint}${path}`, json: true, timeout }, (err, res, body) => {
...
function listRequest (path, cacheDirectory, options = {}, cb) {
  ...
  return GET(path, cacheDirectory, (err, msg) => {
...
function list (path, cacheDirectory) {
  return (options, cb) => listRequest(path, cacheDirectory, options, cb);
}
function itemList (urlTmpl) {
  return (param, options, cb) => listRequest(urlTmpl.replace('{param}', param), cacheDirectory, options, cb);
}
